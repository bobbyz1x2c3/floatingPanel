...
主题（浅色 / 深色 / 跟随系统）、强调色、面板透明度、颗粒质感、磨砂强度、对齐网格、显示网格、完成音效、**音频响应**、窗口置顶、毛玻璃材质、**自动检查更新**，以及整理布局 / 收起全部 / 恢复示例 / 清空卡片。

面板本身就是一块玻璃，默认填充压得比较低（浅色 `--glass-fill: 0.5`、桌面端 0.4），
背景能透出来；嫌太透就拉「面板透明度」那一项。
...
| 位置 | 效果 |
| --- | --- |
| 卡片 | 新卡片依次落下来（带 26ms 递增延迟）；删除时先缩小淡出再消失；跨象限时强调色渐变过去 |
