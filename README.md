...
同一个房间里的人，会在面板背景上看到彼此卡片位置上的**呼吸光点**——每种人一个色相，
点的位置就是对方卡片的相对位置，仅此而已。

| 项 | 说明 |
| --- | --- |
| 传输 | MQTT over WebSocket，房间即 topic：`nemufloat/room/<房间号>/presence` |
| 默认服务器 | `wss://broker.emqx.io:8084/mqtt`（EMQX 公共 broker），联机抽屉里可以换自己的 |
| 心跳 | 每 2.4 秒报一次平安；6 秒收不到就当对方离线；离开时发一个 bye，对面立刻消失 |
| 广播内容 | 随机 id、时间戳、**每张卡片中心点的归一化坐标 + 所在象限** |
| 不广播 | 标题、正文、附件、卡片 id、卡片数量以外的任何内容都不会发出去 |

> ⚠️ 房间号是「知道就能进」的：公共 broker 没有鉴权也没有端到端加密，
> 谁拿到房间号谁就能看到那几个点。放敏感的板子上联机时，请把服务器换成自己的（抽屉里可以改）。
