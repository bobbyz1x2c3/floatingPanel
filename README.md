...
- 卡片右上角是两个图标按钮：收起 / 展开、删除
- 卡片顶部（标题下方）是**更新时间和「完成」按钮**：点完成会播一段收起动效、响一声完成音效，然后这张卡片被归档
- 画板支持滚动；卡片可以拖到画布任意位置
- 内容自动写入 `localStorage`（键名 `nemu-float.board.v1`）

顶栏只有一行：左侧是品牌、新建和搜索，右侧是整理、全部收起、主题、归档（带数量徽标）、设置，以及桌面端的置顶 / 最小化 / 关闭。窗口变窄时按钮文字会先收掉、只留图标，不会折成两行。
...
```
src/
  components/      界面组件（卡片、画板、顶栏、归档抽屉、设置抽屉、控件库、图标）
  lib/             状态机（store.ts）、持久化、平台桥接（platform.ts）、附件、链接识别、图片压缩、格式化、完成音效（sound.ts）
